    ...
    <div class="page-wrapper">
        ...
        <div class="container-fluid">
                                        ...
                                        <div class="col-md-2">
                                            <div class="form-group">
                                                ...
                                                <select name="status" class="form-control">
                                                    <option value="all"
                                                        {{ Request::get('status') == 'all' ? 'selected' : '' }}>All Status
                                                    </option>
                                                    ...
                                                    <option value="deactive"
                                                        {{ Request::get('status') == 'deactive' ? 'selected' : '' }}>
                                                        Deactive</option>
                                    ...
                                    <tbody>
                                        @foreach ($customers as $index => $customer)
                                            <tr id="item{{ $customer->id }}">
                                                <td>{{ $customers->perPage() * $customers->currentPage() - $customers->perPage() + ($index + 1) }}
                                                </td>
                                                <td>{{ $customer->seller_id }}</td>
                                                <td><img src="{{ asset('upload/users') }}/{{ $customer->photo ? $customer->photo : 'default.png' }}"
                                                        width="40"> {{ $customer->name }}
                                                    <p>{{ \Carbon\Carbon::parse($customer->created_at)->format(Config::get('siteSetting.date_format')) }}
                                                    </p>
                                                </td>
                                                <td>{{ $customer->mobile }}</td>
                                                <td>{{ $customer->email }}</td>
                                                <td><a href="{{ route('customer.profile', $customer->username) }}"
                                                        class="label label-info">{{ $customer->posts_count }}</a></td>
                                                <td>{{ $customer->clientIp }}</td>

                                                <td>
                                                    @if ($customer->sellerVerify && $customer->sellerVerify->status == 'active')
                                                        <span style="color:green"> Verified </span>
                                                    @elseif($customer->sellerVerify && $customer->sellerVerify->status == 'pending')
                                                        <span
                                                            style="color:blue">{{ $customer->sellerVerify->status }}</span>
                                                    @elseif($customer->sellerVerify)
                                                        <span
                                                            style="color:red">{{ $customer->sellerVerify->status }}</span>
                                                    @else
                                                ...
                                                <td>
                                                    <span
                                                        class="badge 
                                                            @if ($customer->membership == 'agent') badge-success 
                                                            @elseif($customer->membership == 'authentic') 
                                                                badge-primary 
                                                            @elseif($customer->membership == 'verified') 
                                                                badge-warning 
                                                            @elseif($customer->membership == 'wholesale') 
                                                                badge-info @endif">
                                                        {{ $customer->membership }}
                                                    </span>
                                                </td>
                                                ...
                                                <td>
                                                    <span style="cursor:pointer;"
                                                        class="label @if ($customer->status == 'active') label-success @elseif($customer->status == 'band') label-danger @elseif($customer->status == 'deactive') label-warning @else label-info @endif"
                                                        title="customer Status (pending, active, deactive)"
                                                        @if ($permission['is_edit']) onclick="customerStatus({{ $customer->id }})" @endif>
                                                    ...
                                                    <div class="btn-group">
                                                        ...
                                                        <div class="dropdown-menu">
                                                            ...
                                                            <a class="dropdown-item" title="View Customer Profile"
                                                                data-toggle="tooltip"
                                                                href="{{ route('customer.profile', $customer->username) }}"><i
                                                            ...
                                                            @if ($customer->membership!=null)
                                                                <a class="dropdown-item" target="_blank"
                                                                    ...
                                                                    data-toggle="tooltip"
                                                                     href="{{ route('admin.membershipInvoice', $customer->sellerMembership->id) }}"><i
                                                            ...
                                                            @if ($permission['is_edit'])
                                                                <a class="dropdown-item" target="_blank"
                                                                    ...
                                                                    data-toggle="tooltip"
                                                                    href="{{ route('admin.customerSecretLogin', encrypt($customer->id)) }}"><i
                                                            ...
                                                            @if ($permission['is_delete'])
                                                                <span title="Delete" data-toggle="tooltip"><button
                                                                        data-target="#delete"
                                                                        onclick='deleteConfirmPopup("{{ route('customer.delete', $customer->id) }}")'
                                                            ...
                                                            @if ($permission['is_edit'])
                                                                <button title="Password Reset" data-toggle="tooltip"
                                                                    onclick='PasswordReset("{{ $customer->id }}")'
            ...
            <div class="row">
                ...
                <div class="col-sm-6 col-md-6 col-lg-6 text-right">Showing {{ $customers->firstItem() }} to
                    {{ $customers->lastItem() }} of total {{ $customers->total() }} entries ({{ $customers->lastPage() }}
                    Pages)</div>
            </div>
            <!-- ============================================================== -->
            <!-- End PAge Content -->
