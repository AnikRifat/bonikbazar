...
<div class="row mt-2">
    ...
    @foreach ($items as $item)
        <div class="col-6 col-sm-6 w-100 position-relative p-1">
                ...
                <div class="ppb-5 overflow-hidden h-100">
                    ...
                    <div class="d-flex align-items-center justify-content-between mb-2">
                        <div>
                            ...
                            <p class="bt" title="{{ isset($item['state_name'])? $item['state_name'] : $item['get_state']['name'] }}">{{ isset($item['state_name'])? $item['state_name'] : $item['get_state']['name'] }}</p>
                            <p class="bt hidden-xs" title="{{ $item['category_name'] }}"> {{ $item['category_name'] }}
                                ({{ $item['sale_type'] ? $item['sale_type'] : $item['post_type'] }})</p>
                        </div>
                        @if (isset($item['ribbon']) && $item['ribbon'])
                    <div>
                        <img class="lazyload" width="20" src="{{ asset('upload/images/package/'.$item['ribbon'])}}">
                    </div>
                    @endif
                    </div>
                    <div class="d-flex align-items-center justify-content-between">
            ...
            {{-- <a class="position-absolute bottom-1 hidden-md" href="{{route('user.message', [$item->username, $item->slug])}}" title="Message">
            ...
            <div class="d-flex mt-n3 position-relative z-3">
                ...
                <input type="text" name="message" id="message{{$item->id}}" class="px-2 py-1 w-100 rounded" placeholder="Send message">
                <button @if (Auth::check()) onclick="sendMessage({{$item->id}})" @else data-target="#so_sociallogin" data-toggle="modal" @endif type="button"><img height="23" src="{{ asset('upload/images/sendss.svg')}}"></button>
                </form>
            </div>
        </div> --}}
        </div>
                ...
                @if (count($pinAds) > 0)
                    @foreach ($pinAds as $pinAd)
                        <div class="col-6 col-sm-6 w-100 position-relative p-1">
                                    ...
                                    <div class="d-flex align-items-center justify-content-between mb-2">
                                        <div>
                                            ...
                                            <p class="bt hidden-xs" title="{{ $pinAd->category_name }}">
                                                {{ $pinAd->category_name }}
                                                ({{ $pinAd->sale_type ? $pinAd->sale_type : $pinAd->post_type }})</p>
                                        </div>
                                        @if ($pinAd->ribbon)
                                            <div>
                            ...
                            <div class="hidden-xs">
                                <div class="d-flex mt-n3 position-relative z-3">
                                    <form action="{{ route('user.sendMessage') }}?send=direct" method="post"
                                        class="d-flex align-items-center bb2 rounded shadow mx-3">
                                        @csrf
                                        <input type="hidden" name="productOrConId" value="{{ $pinAd->id }}">
                                        <input type="text" name="message" id="message{{ $pinAd->id }}"
                                            class="px-2 py-1 w-100 rounded" placeholder="Send message">
                                        <button
                                            @if (Auth::check()) onclick="sendMessage({{ $pinAd->id }})" @else data-target="#so_sociallogin" data-toggle="modal" @endif
                                            type="button"><img height="23"
                                                src="{{ asset('upload/images/sendss.svg') }}"></button>
                                    </form>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class=" hidden-xs">
                            <div class="d-flex mt-n3 position-relative z-3">
                                <form action="{{ route('user.sendMessage') }}?send=direct" method="post"
                                    ...
                                    <input type="text" name="message" id="message{{ $product->id }}"
                                        class="px-2 py-1 w-100 rounded" placeholder="Send message">
                                    <button
                                        @if (Auth::check()) onclick="sendMessage({{ $product->id }})" @else data-target="#so_sociallogin" data-toggle="modal" @endif
                                        type="button"><img height="23"
                                            src="{{ asset('upload/images/sendss.svg') }}"></button>
                                </form>
                            </div>
                        ...
                        <div class=" hidden-xs">
                            <div class="d-flex mt-n3 position-relative z-3">
                                <form action="{{ route('user.sendMessage') }}?send=direct" method="post"
                                    ...
                                    <input type="text" name="message" id="message{{ $urgentAd->id }}"
                                        class="px-2 py-1 w-100 rounded" placeholder="Send message">
                                    <button
                                        @if (Auth::check()) onclick="sendMessage({{ $urgentAd->id }})" @else data-target="#so_sociallogin" data-toggle="modal" @endif
                                        type="button"><img height="23"
                                            src="{{ asset('upload/images/sendss.svg') }}"></button>
                                </form>
                            </div>
                        ...
                        <div class=" hidden-xs">
                            <div class="d-flex mt-n3 position-relative z-3">
                                <form action="{{ route('user.sendMessage') }}?send=direct" method="post"
                                    ...
                                    <input type="text" name="message" id="message{{ $highlightAd->id }}"
                                        class="px-2 py-1 w-100 rounded" placeholder="Send message">
                                    <button
                                        @if (Auth::check()) onclick="sendMessage({{ $highlightAd->id }})" @else data-target="#so_sociallogin" data-toggle="modal" @endif
                                        type="button"><img height="23"
                                            src="{{ asset('upload/images/sendss.svg') }}"></button>
                                </form>
                            </div>
                                ...
                                <div class="d-flex align-items-center justify-content-between">
                                    <div>
                                        ...
                                        <p class="bt hidden-xs" title="{{ $fastAd->category_name }}">
                                            ...
                                            ({{ $fastAd->sale_type ? $fastAd->sale_type : $fastAd->post_type }})</p>
                                    </div>
                                    @if ($fastAd->ribbon)
                                        <img width="25" class="lazyload"
                        ...
                        <div class=" hidden-xs">
                            <div class="d-flex mt-n3 position-relative z-3">
                                <form action="{{ route('user.sendMessage') }}?send=direct" method="post"
                                    ...
                                    <input type="text" name="message" id="message{{ $fastAd->id }}"
                                        class="px-2 py-1 w-100 rounded" placeholder="Send message">
                                    <button
                                        @if (Auth::check()) onclick="sendMessage({{ $fastAd->id }})" @else data-target="#so_sociallogin" data-toggle="modal" @endif
                                        type="button"><img height="23"
                                            src="{{ asset('upload/images/sendss.svg') }}"></button>
                                </form>
                            </div>
