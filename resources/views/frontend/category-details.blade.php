...
@section('css')
    <style type="text/css">
        ...
        .product-widget-dropdown .active,
        .parentItem {
            font-weight: 600;
        }
        ...
        .popover {
            max-width: 300px;
            /* Adjust the maximum width of the popover */
        ...
        {
            border-top-color:#353b48 !important;
        }
    </style>
@endsection
@section('content')
    ...
    @php
        ...
        if ($category) {
            if ($category->parent_id && $category->subcategory_id) {
                $maincategory = $category->get_category->get_category->name;
                $maincategory_slug = $category->get_category->get_category->slug;
            } elseif ($category->parent_id) {
                $maincategory = $category->get_category->name;
                $maincategory_slug = $category->get_category->slug;
            } else {
                $maincategory = $category->name;
                $maincategory_slug = $category->slug;
            }

            if ($category->parent_id && $category->subcategory_id) {
                $subcategory = $category->get_category->name;
                $subcategory_slug = $category->get_category->slug;
            } elseif ($category->parent_id) {
    ...
    <div class="container px-0">
        ...
        <div class="row">
            <div style="padding-left: 5px; padding-right: 5px; margin-top: 0.5rem!important;"
                class="col-md-3 @if (!(new \Jenssegers\Agent\Agent())->isDesktop()) filter @endif">
                    ...
                    <div class="accordion w-100" id="accordion">
                        <div class="bg-white mb-3 shadow-b">
                            <div class="bb p-2 shadow-bb" role="tab" id="headingOne">
                                <div class="w-100 d-flex justify-content-between align-items-center" data-toggle="collapse"
                                    data-target="#collapseOne">
                                    <div class="d-flex align-items-center">
                                        <img class="mr-1" width="15" src="{{ asset('upload/images/group.png') }}"
                                            alt="group">
                                        <h4 class="yt">Filter by</h4>
                                    </div>
                                    <img width="15" class="rotate-90" src="{{ asset('upload/images/vector.png') }}"
                                        alt="vector">
                                </div>
                            </div>
                            ...
                            <div id="collapseOne" class="collapse show p-2" role="tabpanel" aria-labelledby="headingOne">
                                ...
                                <div class="d-flex align-items-center">
                                    <input type="checkbox" name="ad" value="premium"
                                        @if (in_array('premium', explode(',', Request::get('ad')))) checked @endif class="common_selector package"
                                        id="premium">
                                ...
                                <div class="d-flex align-items-center">
                                    <input type="checkbox" name="member" value="member"
                                        @if (in_array('member', explode(',', Request::get('member')))) checked @endif class="common_selector member"
                                        id="member">
                                    <label class="iy" for="member">Member Bonik</label>
                                </div>
                                <div class="d-flex align-items-center">
                                    <input type="checkbox" name="member" value="verified"
                                        @if (in_array('verified', explode(',', Request::get('member')))) checked @endif class="common_selector member"
                                        id="verified">
                                    <label class="iy" for="verified">Verified Bonik</label>
                                </div>
                                <div class="d-flex align-items-center">
                                    <input type="checkbox" name="member" value="dealer"
                                        @if (in_array('dealer', explode(',', Request::get('member')))) checked @endif class="common_selector member"
                                        id="dealer">
                                    <label class="iy" for="dealer">Dealers Bonik</label>
                                </div>
                                <div class="d-flex align-items-center">
                                    <input type="checkbox" name="agent" value="agent"
                                        @if (in_array('agent', explode(',', Request::get('member')))) checked @endif class="common_selector member"
                                        id="agent">
                                    <label class="iy" for="agent">Agent Bonik</label>
                                </div>
                                <div class="d-flex align-items-center">
                                    <input type="checkbox" name="member" value="wholesale"
                                        @if (in_array('wholesale', explode(',', Request::get('member')))) checked @endif class="common_selector member"
                                        id="wholesale">
                                        ...
                                        @if ($category && Request::route('catslug'))
                                            @if ($category->parent_id == $show_category->id || $category->id == $show_category->id)
                                                <li class="parentItem"><a
                                                        href="{{ Request::route('location') ? route('home.category', [Request::route('location'), $show_category->slug]) : route('home.category', ['all', $show_category->slug]) }}{{ request()->getQueryString() ? '?' . request()->getQueryString() : null }}"><img
                                                            alt=""
                                                            src="{{ asset('upload/images/category/thumb/' . $show_category->image) }}"><span>{{ $show_category->name }}
                                                            <p>{{ $show_category->products_by_category_count }} Ads</p>
                                                            </span> </a></li>
                                                ...
                                                @if (Request::route('catslug'))
                                                    <ul class="product-widget-dropdown" style="display: block;">
                                                        @foreach ($show_category->get_subcategory as $filterCategory)
                                                            <li @if ($category->id == $filterCategory->id) class="active" @endif><a
                                                                    href="{{ Request::route('location') ? route('home.category', [Request::route('location'), $filterCategory->slug]) : route('home.category', ['all', $filterCategory->slug]) }}{{ request()->getQueryString() ? '?' . request()->getQueryString() : null }}"><span>
                                                                        {{ $filterCategory->name }} <p>
                                                                            {{ $filterCategory->products_by_subcategory_count }}
                                                                            Ads</p></span> </a></li>
                                                        @endforeach
                                                    </ul>
                                                @endif
                                        ...
                                        @else
                                            <li><a
                                                    href="{{ Request::route('location') ? route('home.category', [Request::route('location'), $show_category->slug]) : route('home.category', ['all', $show_category->slug]) }}{{ request()->getQueryString() ? '?' . request()->getQueryString() : null }}"><img
                                                        alt=""
                                                        src="{{ asset('upload/images/category/thumb/' . $show_category->image) }}"><span>{{ $show_category->name }}
                                                        <p>{{ $show_category->products_by_category_count }} Ads</p></span>
                                                </a></li>
                                        @endif
                                    @endforeach
                        ...
                        <div class="bg-white mb-3 shadow-b">
                            <div class="bb p-2 shadow-bb" role="tab" id="headingOne">
                                <div class="w-100 d-flex justify-content-between align-items-center"
                                    data-toggle="collapse" data-target="#location">
                                    <div class="d-flex align-items-center">
                                        <img class="mr-1" width="15" src="{{ asset('upload/images/group.png') }}"
                                            alt="group">
                            ...
                            <div id="location" class="collapse show p-2" role="tabpanel" aria-labelledby="headingOne">
                                <ul class="product-widget-list">
                                    ...
                                    <li><a
                                            href="{{ Request::route('location') ? route('home.category', [Request::route('catslug')]) : route('home.category') }}">
                                            All Location</a></li>
                                    ...
                                    @foreach ($states as $show_state)
                                        ...
                                        @if ($state)
                                            @if ($state->state_id == $show_state->id || ($state->id == $show_state->id && $state->state_id == null))
                                                <li class="parentItem"><a
                                                        href="{{ Request::route('catslug') ? route('home.category', [$show_state->slug, Request::route('catslug')]) : route('home.category', $show_state->slug) }}{{ request()->getQueryString() ? '?' . request()->getQueryString() : null }}"><span>
                                                            {{ $show_state->name }}<p>
                                                                {{ $show_state->products_by_state_count }} Ads</p></span>
                                                    </a></li>

                                                @if ($show_state->get_city && Request::route('location') && Request::route('location') != 'all')
                                                    <ul class="product-widget-dropdown" style="display: block;">
                                                        @foreach ($show_state->get_city as $city)
                                                            <li @if ($state->id == $city->id && $state->state_id != null) class="active" @endif><a
                                                                    href="{{ Request::route('location') ? route('home.category', [$city->slug, Request::route('catslug')]) : route('home.category', $city->slug) }}{{ request()->getQueryString() ? '?' . request()->getQueryString() : null }}"><span>
                                                                        {{ $city->name }}<p>
                                                                            {{ $city->products_by_city_count }} Ads</p>
                                                                        </span> </a></li>
                                                        @endforeach
                                                    </ul>
                                                @endif
                                        ...
                                        @else
                                            <li><a
                                                    href="{{ Request::route('catslug') ? route('home.category', [$show_state->slug, Request::route('catslug')]) : route('home.category', $show_state->slug) }}{{ request()->getQueryString() ? '?' . request()->getQueryString() : null }}"><span>
                                                        {{ $show_state->name }}<p>
                                                            {{ $show_state->products_by_state_count }} Ads</p></span> </a>
                                            </li>
                                        @endif
                                    @endforeach
                            ...
                            <div class="bg-white mb-3 shadow-b">
                                <div class="bb p-2 shadow-bb" role="tab" id="headingOne">
                                    <div class="w-100 d-flex justify-content-between align-items-center"
                                        ...
                                        <div class="d-flex align-items-center">
                                            <img class="mr-1" width="15"
                                                src="{{ asset('upload/images/group.png') }}" alt="group">
                                            <h4 class="yt">Brand</h4>
                                        </div>
                                        <img width="15" class="rotate-90"
                                            src="{{ asset('upload/images/vector.png') }}" alt="vector">
                                    </div>
                                </div>

                                <div id="collapseOne" class="collapse show p-2" role="tabpanel"
                                    aria-labelledby="headingOne">
                                    @foreach ($brands as $brand)
                                        <div class="d-flex align-items-center">
                                            <input @if (in_array($brand->slug, explode(',', Request::get('brand')))) checked @endif
                                                class="common_selector brand" value="{{ $brand->slug }}"
                        ...
                        @foreach ($product_variations as $product_variation)
                            ...
                            @if (count($product_variation->get_attrValues) > 0)
                                <div class="bg-white mb-3 shadow-b">
                                    ...
                                    <div id="collapse3" class="collapse show p-2" role="tabpanel"
                                        aria-labelledby="heading3">
                                        @foreach ($product_variation->get_attrValues as $variationValue)
                                            <div class="d-flex align-items-center">
                                                <input @if (in_array(strtolower($variationValue->name), explode(',', Request::get(strtolower($product_variation->name))))) checked @endif
                                                    value="{{ strtolower($variationValue->name) }}"
                                                    class=" {{ str_replace(' ', '', $product_variation->name) }} common_selector"
                                                    id="attr{{ $variationValue->id }}" type="checkbox">
                                                <label class="iy"
                                                    for="attr{{ $variationValue->id }}">{{ $variationValue->name }}</label>
                                            </div>
                                        @endforeach
                                    </div>
                                </div>
                    ...
                    <div id="price" class="collapse show p-2" role="tabpanel" aria-labelledby="headingOne">
                        <form class="product-widget-form">
                            <div class="product-widget-group">
                                <input type="text" value="{{ Request::get('price_min') }}" id="price_min"
                                    class="price-range" placeholder="min - 00">
                                <input type="text" value="{{ Request::get('price_max') }}" id="price_max"
                                    placeholder="max - 1B">
                            </div>
                            ...
                            <li class="product-widget-dropitem"
                                ...
                                <ul class="product-widget-dropdown">
                                    @foreach ($category->get_subcategory as $subcategory)
                                        <li><a
                                                href="{{ Request::route('location') ? route('home.category', [Request::route('location'), $subcategory->slug]) : route('home.category', ['all', $subcategory->slug]) }}{{ request()->getQueryString() ? '?' . request()->getQueryString() : null }}">
                            ...
                            <li class="product-widget-dropitem"
                                ...
                                <ul class="product-widget-dropdown">
                                    @foreach ($state->get_city as $city)
                                        <li><a
                                                href="{{ Request::route('location') ? route('home.category', [$city->slug, Request::route('catslug')]) : route('home.category', $city->slug) }}{{ request()->getQueryString() ? '?' . request()->getQueryString() : null }}">
                                                {{ $city->name }}</a></li>
                                    @endforeach
                                </ul>
                            </li>
