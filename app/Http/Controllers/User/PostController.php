...
class PostController extends Controller
{
    ...
    public function create(Request $request, string $post_id=null, string $category=null){
        ...
        if($request->isMethod('post') && !$category){
            ...
            if($free_ads->status == 1){
                ...
                //check post limit
                $free_ads_limit = (Auth::user()->membership) ? 100 : (($category->free_ads_limit > 0) ? $category->free_ads_limit : 10);
    ...
    public function updateWantedPost(Request $request, $id)
    {
        ...
        if($store) {
            ...
            //redirect post list page
            return redirect()->route('post.list')->with('success', 'Post update successfully.');
        }else{
            Toastr::error('Post Cannot Update.!');
        }
        return back();
    }
