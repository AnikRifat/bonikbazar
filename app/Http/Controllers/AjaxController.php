...
class AjaxController extends Controller
{
    ...
    //ads view count
    public function adViewCount($id){
        $ads = Addvertisement::find($id);
        if($ads){
            $views = rand(1, 5);
            $ads->views = $ads->views + $views;
            $ads->save();
        }
        return true;
    }    
    ...
    {
        $field = ($request->field) ? $request->field : 'image';
        $data = DB::table($request->table)->where('id', $request->id)->first();
        if($data){
            if($request->field){
                DB::table($request->table)->where('id', $request->id)->update([$field => null]);
            }else{
                $image = public_path('upload/images/product/gallery/'.$data->image_path);
                if(file_exists($image) && $data->image_path){
                    unlink($image);
                }
