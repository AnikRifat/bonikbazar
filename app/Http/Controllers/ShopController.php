...
class ShopController extends Controller
{
    ...
    public function userProfile($username){
        $data['user'] = User::with(["sellerVerify","getMembership"])->where('username', $username)->first();
        ...
        if($data['user']){
            ...
            $data['posts'] = Product::where('user_id', $data['user']->id)->where('status', 'active')->orderBy('views', 'desc')->paginate(12);
