...
class PackagePurchaseController extends Controller
{
    ...
    public function packagePurchase(Request $request, $id){
        ...
        if($package){
            ...
            $order_id = $this->uniqueOrderId('package_purchases', 'order_id', 'B');
    ...
    public function packagePurchasePayment(Request $request, $id){
        ...
        $package = PackagePurchase::where('order_id', $id)->first();
        if($package){
    ...
    {
        $orders = PackagePurchase::with(['get_package', 'get_category', 'get_boostAd'])->where('payment_method', '!=', 'pending')->where('user_id', Auth::id());
